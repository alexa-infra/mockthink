...
class TestGroupAggregation(MockTest):
    @staticmethod
    def get_data():
        data = [
            {'id': 'joe', 'type': 'bro'},
    ...
    def test_count(self, conn):
        expected = [
            {'group': 'bro', 'reduction': 2},
            {'group': 'hipster', 'reduction': 2},
            {'group': 'unknown', 'reduction': 1},
        ]
        query = r.db('x').table('people')
        query = query.group('type')
        query = query.count()
        query = query.ungroup()
        results = query.run(conn)
        assertCountEqual(expected, results)
